...
from utils.utils import (
    glmm5CV_encode_multiple_features_gpboost,
    TargetEncoderMultiClass,
)
...
def process_dataset(
    ...
    if dataset_name == "churn":
        ...
        numeric_cols = list(
            set(df.columns) - set([y_col]) - set(bin_cols + cat_cols + z_cols)
        )
        ...
        bin_cols = list(
            set(df.nunique()[df.nunique() == 2].index)
            - set(["Who_Pays_for_Access_Work"])
        )
        cat_cols = list(
            set(df.nunique()[df.nunique() < hct].index) - set([y_col]) - set(bin_cols)
        )
        numeric_cols = []
        ...
        bin_cols = ["sex"]
        cat_cols = list(
            set(df.dtypes[df.dtypes == "object"].index) - set(["sex", "class"] + z_cols)
        )
        ...
        cat_cols = [
            ...
            if np.logical_and(
                np.logical_and(df[col].nunique() > 2, df[col].nunique() < hct),
                "_cat" in col,
            )
        ]
        z_cols = [
            col
            for col in df.columns
            if np.logical_and(df[col].nunique() >= hct, "_cat" in col)
        ]
        # 5. Rest is numeric
        numeric_cols = list(
            set(df.columns[df.dtypes != "object"])
            - set([y_col])
            - set(bin_cols + cat_cols + z_cols)
        )
        # 6. Label encode dtypes==object
        ...
        z_cols = list(
            df.columns[list(np.logical_and(df.nunique() >= hct, df.dtypes == "object"))]
        )
        ...
        df = df.drop(
            ["datetime", "PM.sub.2.5..sub..particulate.matter..Hourly.measured."],
            axis=1,
        )
    ...
    assert (
        len(cat_cols + [y_col] + z_cols + bin_cols + numeric_cols) == df.shape[1]
    ), "Column type definitions imply different dimensionality than dataset"
    ...
    if mode == "cv":
        ...
        split = [
            (
                np.array(list(set(X.index).difference(test_indices))),
    ...
    for num, (train_indices, test_indices) in enumerate(split):
        ...
        bin_impute = {}
        for col in (
            cat_cols + z_cols + bin_cols
        ):  # [i for i in bin_cols if df[i].dtype == "object"]:
            le_ = OrdinalEncoder(
                handle_unknown="use_encoded_value",
                unknown_value=-2,
                encoded_missing_value=-1,
            )
            X_train[col] = le_.fit_transform(
                X_train[col].astype(str).values.reshape(-1, 1)
            )
            if mode in ["train_val_test", "cv"]:
        ...
        if target == "continuous":
            ...
            if mode in ["train_val_test", "cv"]:
                y_val = pd.Series(
                    target_scaler.transform(y_val.values.reshape(-1, 1)).ravel(),
                    index=X_val.index,
                )
            y_test = pd.Series(
        ...
        if len(z_cols) > 0:
            ...
            for col in z_cols:
                ...
                ohe_encoded_train = pd.DataFrame(
                    enc.fit_transform(X_train[[col]]).toarray(),
                    columns=enc.get_feature_names_out([col]),
                    index=X_train.index,
                )
                if mode in ["train_val_test", "cv"]:
                    ohe_encoded_val = pd.DataFrame(
                        enc.transform(X_val[[col]]).toarray(),
                        columns=enc.get_feature_names_out([col]),
                        index=X_val.index,
                    )
                ohe_encoded_test = pd.DataFrame(
                    enc.transform(X_test[[col]]).toarray(),
                    columns=enc.get_feature_names_out([col]),
                    index=X_test.index,
                )
            ...
            for col in z_cols:
                ...
                re_encoded_train = encoder.fit_transform(
                    X_train[col].astype(object), y_train
                )
                if mode in ["train_val_test", "cv"]:
        ...
        if any(z_target_encoded_train.nunique() == 1):
            drop_cols = z_target_encoded_train.columns[
                z_target_encoded_train.nunique() == 1
            ]
            ...
            z_target_encoded_test = z_target_encoded_test.drop(
                z_target_encoded_test.columns[z_target_encoded_test.nunique() == 1],
                axis=1,
            )
        ...
        if len(cat_cols) > 0:
            ...
            encoded_train = pd.DataFrame(
                enc.fit_transform(X_train[cat_cols]).toarray(),
            ...
            if mode in ["train_val_test", "cv"]:
                encoded_val = pd.DataFrame(
                    enc.transform(X_val[cat_cols]).toarray(),
                    columns=enc.get_feature_names_out(cat_cols),
                    index=X_val.index,
                )
            ...
            encoded_test = pd.DataFrame(
                enc.transform(X_test[cat_cols]).toarray(),
                columns=enc.get_feature_names_out(cat_cols),
                index=X_test.index,
            )
        ...
        # GLMM Encoding
        qs = [
            np.max([Z_train[col].max(), Z_val[col].max(), Z_test[col].max()]) + 1
    ...
    with open(
        f"../data/prepared/{dataset_name}/{new_path}/data_dict.pickle", "wb"
    ) as handle:
